...
from code_references import CODE_REGEX, filter_code_regex
from matching import get_matching_results_dict, get_matching_result_item, switch_pattern, get_code_refs
...
class TestMatching:
    ...
    @pytest.mark.parametrize("input_expected", [
        ("Comme il est mentionné dans l'article 238-4 alinéa 2 du code de la consommation .", ["Code de la consommation","238-4"]),
        ("Comme il est mentionné dans l'art.238-4 alinéa 2 du C.conso.", ["Code de la consommation","238-4"]),
        ("Art. 2-3-4 du Code de l'Environnement", ["Code de l'environnement", "2-3-4"]),
        ("Art. R2-3-4 du CE.", ["Code de l'environnement", "R2-3-4"]),
        ("\n-\n article L.278 du cjaaa", ["Code de justice administrative", "L278"]),
        ("Art. L. 385-2, R. 343-4 et A421-13 du C. assur. ", [["Code des assurances", "L385-2"], ["Code des assurances","R343-4"], ["Code des assurances", "A421-13"]]) 
    ])
    ...
    @pytest.mark.parametrize("input_expected", [
        ("Comme il est mentionné dans le code de la consommation article 238-4 alinéa 2.", ["Code de la consommation","238-4"]),
        ("Comme il est mentionné dans  le C.conso. art.238-4 alinéa 2  qui blablabla", ["Code de la consommation","238-4"]),
        ("Code de l'Environnement Art. 2-3-4", ["Code de l'environnement", "2-3-4"]),
        ("CE. Art. R2-3-4", ["Code de l'environnement", "R2-3-4"]),
        ("\n-\n  cjaaa article L.278 ", ["Code de justice administrative", "L278"]),
        ("C. assur. Art. L. 385-2, R. 343-4 et A421-13", [["Code des assurances", "L385-2"], ["Code des assurances","R343-4"], ["Code des assurances", "A421-13"]])
    ])
    ...
    def test_get_code_refs_reversed_pattern(self):
        full_text = "CSI Article LL22-7 et R314-7  Code de l'environnement L124-1 CJA L214-4"
        
        results = [n for n in (get_code_refs(full_text, "code_article", selected_codes=None))]
    ...
    def test_matching_result_dict_codes_no_filter_pattern_code_article(self):
        ...
        code_reference_test_002 = CODE_REGEX
        ...
        for file_path in file_paths:
            ...
            restored_abspath = restore_test_file(file_path)
            results_dict = get_matching_results_dict(
                full_text, None, "article_code"
            )
            assert sorted(list(results_dict.keys())) == sorted(code_reference_test_002), sorted(code_reference_test_002)
            assert len(results_dict) == 16, len(results_dict)
            assert len(list(results_dict.values())) == 16, len(list(results_dict.values()))
            assert results_dict["CCIV"] == ['1120', '2288', '1240-1', '1140', '1', '349', '39999'], results_dict["CCIV"]
            # assert results_dict["CASSUR"] == ["L385-2", "R343-4", "A421-13"], results_dict[
            #     "CASSUR"
            # ]
            # assert results_dict["CSI"] == ["L622-7", "R314-7"], results_dict["CSI"]
